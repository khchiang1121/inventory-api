...
### 4.2 後端 Resource Services（可分區部署、僅負責實作）
...
* VM / K8s 建立請求 → 檢查配額與資源 → 自動選擇 Baremetal → 分派至後端 Service → 回報狀態 → 更新資料庫。
...
本設計已對齊公有雲實務標準，具備高一致性、強解耦性與可觀測性，適合作為私有雲平台的 API 中樞架構。推薦持續優化以下方向：
