...
def test_crud_fabrication(auth_client):
    ...
    # Full Update (PUT)
    put_payload = {"name": "fab-a-updated", "old_system_id": "legacy-updated"}
    r = auth_client.put(f"/api/v1/fabrications/{fab_id}", put_payload, format="json")
    assert r.status_code == 200
    assert r.data["name"] == "fab-a-updated"
    assert r.data["old_system_id"] == "legacy-updated"
    ...
    # Name should remain unchanged from PUT
    assert r.data["name"] == "fab-a-updated"
...
def test_crud_phase(auth_client):
    ...
    # Full Update (PUT)
    put_payload = {"name": "phase-1-updated", "old_system_id": "p-1-updated"}
    r = auth_client.put(f"/api/v1/phases/{obj_id}", put_payload, format="json")
    assert r.status_code == 200
    assert r.data["name"] == "phase-1-updated"
...
def test_crud_data_center(auth_client):
    ...
    # Full Update (PUT)
    put_payload = {"name": "DC1-updated", "old_system_id": "d-1-updated"}
    r = auth_client.put(f"/api/v1/data-centers/{obj_id}", put_payload, format="json")
    assert r.status_code == 200
    assert r.data["name"] == "DC1-updated"
...
def test_crud_room(auth_client):
    ...
    # Full Update (PUT)
    put_payload = {"name": "R1-updated", "old_system_id": "r-1-updated"}
    r = auth_client.put(f"/api/v1/rooms/{obj_id}", put_payload, format="json")
    assert r.status_code == 200
    assert r.data["name"] == "R1-updated"
...
def test_crud_rack(auth_client):
    ...
    put_payload = {
        ...
        "bgp_number": "65001",
        "as_number": 65001,
        "old_system_id": "rack-1-updated",
        "height_units": 44,
        "power_capacity": "6.00",
        "status": "active",
    }
    r = auth_client.put(f"/api/v1/racks/{obj_id}", put_payload, format="json")
    assert r.status_code == 200
    assert r.data["name"] == "RACK-A-updated"
